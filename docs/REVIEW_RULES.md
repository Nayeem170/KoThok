...
- [seed] "Do not test it": once a seam is collapsed to one source, a test asserting the two former sites agree is either tautological (both now read the shared source, so it validates the source against itself) or anchored to nothing (it checks the duplication is internally consistent, which holds even when both copies drift wrong together). The structural identity is the guarantee; the test adds nothing, and a green on it can mask the next regression.
