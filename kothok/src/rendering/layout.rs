...
pub use kobo_core::rendering::layout::{
    sentences_with_ranges, word_wrap_bytes, word_wrap_char_based_styled, word_wrap_indent,
};
pub use state::*;
...
/// `overlay_text` so wrap measurement and draw agree. If they diverge, lines
/// either overflow the column or end ragged.
pub const MONO_SCALE: f32 = 0.8;

/// Heading size as a multiple of the body font size, by level.
///
/// Shared by `push_heading_rows` (which wraps and sizes the rows) and
/// `overlay_text` (which draws them). Two copies of this table is how headings
...
pub const ROW_FLAG_BQ: i32 = 8;
/// Centre the row in the text column. Figure captions only.
pub const ROW_FLAG_CENTER: i32 = 16;
...
pub struct ChapterState {
    ...
    /// Hyperlink destinations over the same `body` offsets. Kept apart from
    /// `style_runs` because it owns a `String` per entry and only the tap
    /// handler reads it, while `style_runs` is walked on every repaint.
    pub links: Vec<kobo_core::html_text::LinkRun>,
}

impl ChapterState {
    /// The link covering a `body` offset, if any.
    pub fn link_at(&self, offset: usize) -> Option<&kobo_core::html_text::LinkRun> {
        self.links
            .iter()
    ...
    pub fn page_for_offset(&self, offset: usize) -> Option<usize> {
        self.pages.iter().position(|(s, e)| {
            self.all_rows.get(*s..*e).is_some_and(|rows| {
                rows.iter().any(|r| {
                    r.start < r.end && offset >= r.start as usize && offset < r.end as usize
                })
            })
        })
    }
