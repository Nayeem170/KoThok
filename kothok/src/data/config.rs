...
mod tests {
    ...
    /// Opting out of the update check must survive a save/load cycle -- it is
    /// the only setting that governs an outbound network request, so silently
    /// reverting it to the default would re-enable something switched off.
    #[test]
    fn update_check_opt_out_survives_a_roundtrip() {
        let p = tmp_path("updates");
        let cfg = AppConfig {
            check_updates: false,
            ..AppConfig::default()
        };
        save_config_to(&cfg, &p);
        let loaded = load_config_from_base(&p, 36);
        assert!(!loaded.check_updates, "opt-out must persist");
        let _ = std::fs::remove_file(&p);
    }

    #[test]
    fn update_check_defaults_to_on() {
        assert!(AppConfig::default().check_updates);
    }
