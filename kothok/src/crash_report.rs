...
pub fn check_on_startup(device_model: &str) {
    ...
    let content = std::fs::read_to_string(config::CRASH_LOG).unwrap_or_default();
    if content.trim().is_empty() {
        return;
    }

    log::info!("crash-report: previous crash detected, will upload when WiFi connects");
...
fn upload(device: &str, content: &str) -> bool {
    let body = serde_json::json!({
        ...
        "subject": format!("KoThok crash report - {BUILD_TAG}"),
        "from_name": device,
        "Build": BUILD_TAG,
        "message": content,
    });
        ...
        .send_string(&body.to_string())
    {
        Ok(resp) => {
            let success = resp
                ...
                .unwrap_or(false);
            if success {
                log::info!("crash-report: uploaded, clearing crash.log");
                let _ = std::fs::write(config::CRASH_LOG, "");
            } else {
                log::warn!("crash-report: server rejected upload");
            }
            success
        }
        Err(e) => {
            log::warn!("crash-report: upload failed: {e}");
            false
        }
    }
}
