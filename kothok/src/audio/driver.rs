...
struct ReadyUtt {
    idx: usize,
    prep: Rc<kobo_core::audio::Prepared>,
    start: usize,
    end: usize,
    para_end: bool,
...
struct DriverState {
    ...
    /// Cache key computed at synth start and carried through to completion, so
    /// the store path does not need to re-derive the voice from text.
