...
impl DriverState {
    ...
    pub(super) fn try_prefetch(&mut self) {
        ...
        self.pending_cache_key = Some((text, utt_voice, rate));
        self.idx += 1;
    }

    /// Abort and retry a hung synth (websocket stall leaves `pending` forever).
    ...
    /// Retry the failed utterance. `try_prefetch` already advanced `idx`, so
    /// rewind to avoid silently dropping content. Caller must clear `pending`.
        ...
        match h.await {
            Ok(Ok(prep)) => {
                ...
                if let Some((s, e, para_end)) = self.pending_range.take() {
                    ...
                    let prep = Rc::new(prep);
                    if let Some(ck) = self.pending_cache_key.take() {
                        self.cache.store(ck, Rc::clone(&prep));
                    }
                    let ru = ReadyUtt {
                        idx: synth_idx,
                        prep,
    ...
    pub(super) async fn idle(&mut self) {
        ...
        if let Some(p) = self.player.as_mut() {
            ...
            {
                let t0 = Instant::now();
                if let Err(_) = p.keepalive().await {
                    drop_sink = true;
                } else {
