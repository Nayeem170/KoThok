...
/// Seeking to a later page must not land on the first utterance.
///
/// Regression guard: `apply_page_display` used to pass `pages[p].0` -- a **row**
/// index -- to `utterance_index_for_offset`, which expects a **byte** offset.
/// Row indices are small enough that the first utterance always satisfies
/// `start >= offset`, so every page silently seeked to the top of the chapter.
#[test]
fn seeking_a_later_page_does_not_land_on_the_first_utterance() {
    let state = make_chapter_state();
    let page0 = first_utt_on_page(&state, 0);
    let page1 = first_utt_on_page(&state, 1);
    assert_eq!(page0, 0, "page 0 starts at the first utterance");
    assert!(
        page1 > page0,
        "page 1 must start later in the queue, got {page1} for page 1 vs {page0} for page 0"
    );
}

/// The bug itself, pinned: feeding the offset lookup a row index resolves
/// *earlier* than the page really starts, which is why the defect played from
/// the wrong place instead of failing loudly. Row indices are always far
/// smaller than the byte offsets they index into, so the error is one-directional.
#[test]
fn a_row_index_seeks_earlier_than_the_page_starts() {
    let state = make_chapter_state();
    let correct = first_utt_on_page(&state, 1);
    let row_index = state.pages[1].0;
    let from_row_index = utterance_index_for_offset(&chapter_utterances(&state), row_index);
    assert!(
        from_row_index < correct,
        "row index {row_index} resolved to utterance {from_row_index}, \
         but page 1 really starts at utterance {correct}"
    );
}
