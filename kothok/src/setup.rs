...
pub fn run() -> Option<InitResult> {
    ...
    let st = build_loop_state(
        ...
        h,
        hw_cfg.model,
        setup.reader.get_wifi_on(),
        setup.reader.get_bt_on(),
    );
...
fn scan_and_resolve(
    ...
    let titles: Vec<String> = all_books.iter().map(|b| b.title.clone()).collect();
    std::thread::spawn(move || {
        for t in &titles {
            crate::device::fonts::preload_cjk_for_titles(t);
        }
    });
...
fn detect_hardware() -> (hw::DeviceConfig, hw::InputDevices) {
    let mut hw_cfg = hw::detect_device().unwrap_or_else(|| {
        ...
        crate::device::registry::DEVICES
            ...
            .expect("device registry must contain monza default")
            .clone()
    });
