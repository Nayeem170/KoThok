                ...
                if let Some(page) = st.state.page_for_offset(start) {
                    if page != st.current_page {
                        ...
                        reader.set_saved_page(
                            (*st.chapter_offsets.get(st.current_chapter).unwrap_or(&0)
                                + st.current_page) as i32,
                        );
                        text_dirty = true;
                    }
                }
...
fn handle_audio_ended(st: &mut LoopState, reader: &Reader, cmd_tx: &Sender<Cmd>) -> (bool, bool) {
    ...
    if st.current_page + 1 < st.state.pages.len() {
        ...
        reader.set_saved_page(
            (*st.chapter_offsets.get(st.current_chapter).unwrap_or(&0) + st.current_page) as i32,
        );
        text_dirty = true;
...
/// Clamp a page index to a chapter's real page count.
///
/// `chapter_offsets` starts out as an estimate and is refined asynchronously,
/// so a progress-bar target can name a page past the end of the chapter it
/// resolves to.
fn clamp_page(page: usize, state: &crate::rendering::layout::ChapterState) -> usize {
    page.min(state.pages.len().saturating_sub(1))
}
...
fn apply_page_display(st: &LoopState, reader: &Reader, cmd_tx: &Sender<Cmd>) {
    ...
    if matches!(st.view_mode, crate::ViewMode::Audio) {
        ...
        //
        // `pages[p]` holds *row* indices, not byte offsets, so it cannot be
        // handed to an offset lookup: a row index is small enough that the
        // first utterance always satisfies `start >= offset` and every page
        // seeked to the top of the chapter. `first_utt_on_page` does the
        // row-to-offset conversion itself.
        let target = crate::audio::glue::first_utt_on_page(&st.state, st.current_page);
...
fn apply_progress_target(
    ...
    // `local` is a page index within the *target* chapter, so it can only be
    // clamped once that chapter's state exists. Clamping first used the page
    // count of the chapter being left: a drag from a short chapter into a long
    // one landed a few pages into it, which reads as "it jumped to the start of
    // the chapter".
    if c != st.current_chapter {
        switch_to(st, reader, cmd_tx, c, false);
        st.current_page = clamp_page(local, &st.state);
        apply_page_display(st, reader, cmd_tx);
        o.navigated = true;
    } else {
        let lp = clamp_page(local, &st.state);
