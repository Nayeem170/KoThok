...
pub struct LoopState {
    ...
    /// The chapter-overlay list, flattened from the book's TOC tree (issue
    /// 11) -- or one row per spine chapter when it has none. Flattened once
    /// here rather than per repaint: the paint and hit-test both need a flat,
    /// indexable sequence, and a tree can't serve either directly.
    pub toc_rows: Vec<FlatTocRow>,
    ...
    /// User's radio intent: true when the user turned WiFi/BT on via the panel,
    /// false when they turned it off. Independent of live connection status
    /// (which `refresh_status` writes to `reader.wifi_on`/`reader.bt_on`).
    /// Sleep/lock/wake key off these so a dropped connection during sleep still
    /// reconnects on wake, instead of seeing a false flag and skipping reconnect.
    /// At boot these are seeded from live connection status (what nickel left
    /// running), which is an acceptable proxy for user intent.
    pub wifi_user_on: bool,
    pub bt_user_on: bool,
    ...
    /// 2x magnifying-glass zoom of the reading page, toggled by a content-area
    /// double-tap. `zoom_center` is the absolute pixel the crop is centred on,
